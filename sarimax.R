...
seasonal_period <- 24  # sazonalidade de 24 horas

order_sarima <- matrix(NA, m, 7)
colnames(order_sarima) <- c("p", "d", "q", "P", "D", "Q", "period")

metric_cols <- c("SARIMAX-TH","SARIMAX-Temp","SARIMAX-H","SARIMA","SARIMAX-DUM")
MAE <- MAPE <- RMSE <- COR <- matrix(NA, m, length(metric_cols))
colnames(MAE) <- colnames(MAPE) <- colnames(RMSE) <- colnames(COR) <- metric_cols

rownames(order_sarima) <- rownames(MAE) <- rownames(MAPE) <- rownames(RMSE) <- rownames(COR) <- sensors
...
plots_dir <- "plots_hour_test_sarimax"
...
for (i in seq_along(sensors)) {
  ...
  RSSI_ts <- ts(RSSI, frequency = seasonal_period)
  RSSI_test_ts <- ts(RSSI_test, frequency = seasonal_period)
  ...
  order_part <- ord[1:3]
  seasonal_part <- list(order = ord[4:6], period = ord[7])
  ...
  a02 <- Arima(RSSI_ts, order = order_part, seasonal = seasonal_part, xreg = Xtemp)
  a03 <- Arima(RSSI_ts, order = order_part, seasonal = seasonal_part, xreg = Xhum)
  a04 <- Arima(RSSI_ts, order = order_part, seasonal = seasonal_part)
  a05 <- Arima(RSSI_ts, order = order_part, seasonal = seasonal_part, xreg = Xdum)
  
  
  # one-step-ahead---- 
  
  new1 <- Arima(RSSI_test_ts, xreg = Xth_t, model = a01)
  new2 <- Arima(RSSI_test_ts, xreg = Xtemp_t, model = a02)
  new3 <- Arima(RSSI_test_ts, xreg = Xhum_t, model = a03)
  new4 <- Arima(RSSI_test_ts, model = a04)
  new5 <- Arima(RSSI_test_ts, xreg = Xdum_t, model = a05)
  ...
  # =========================
  # PLOT (1 por sensor): Observado + todos os modelos (TESTE) - one-step-ahead
  # =========================
  
  plot_df <- data.frame(
    rdtimestamp = as.POSIXct(df_te$rdtimestamp),
    Observed       = as.numeric(RSSI_test),
    `SARIMAX-TH`   = as.numeric(new1$fitted),
    `SARIMAX-Temp` = as.numeric(new2$fitted),
    `SARIMAX-H`    = as.numeric(new3$fitted),
    `SARIMA`       = as.numeric(new4$fitted),
    `SARIMAX-DUM`  = as.numeric(new5$fitted)
  )
  ...
  #   `SARIMAX-TH`   = as.numeric(pred1),
  #   `SARIMAX-Temp` = as.numeric(pred2),
  #   `SARIMAX-H`    = as.numeric(pred3),
  #   `SARIMA`       = as.numeric(pred4),
  #   `SARIMAX-DUM`  = as.numeric(pred5)
  # )
...
print(cbind(order_sarima, Xsig))



base_col <- which(metric_cols == "SARIMA")
comp_cols <- which(metric_cols %in% c("SARIMAX-TH","SARIMAX-Temp","SARIMAX-H","SARIMAX-DUM"))
...
count <- apply(cbind(apply(result01[1:3,], 1, rank) == 1,
                     COR = rank(result01[4,]) == length(metric_cols)), 1, sum)

for (i in 2:m) {
  r <- get(paste0("result0", i))
  r <- apply(cbind(apply(r[1:3,], 1, rank) == 1,
                   COR = rank(r[4,]) == length(metric_cols)), 1, sum)
